...
def compute_metrics(units_df, session, args):
    ...
    presentation_ids = session.stimulus_presentations.loc[
            (session.stimulus_presentations['stimulus_name'] == args['natural_movies']['stimulus_key'])
            ].index.values
    ...
    return units_df.assign(nm_image_selectivity = lambda df: compute_image_selectivity(df.unit_id, conditionwise_response_dataframe) \
                            )
